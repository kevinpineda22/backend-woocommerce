...
The block used to be worse than dormant: it read the session's logs with `.eq("id_sesion", …)`, and **`wc_log_picking` has no `id_sesion` column** (see `sql/2026-08-19_hot_path_indexes.sql`) — logs hang off the session through `id_asignacion`. The query failed silently, `allSessionLogs` came back null, and waking the block would have marked *every collected item* `no_encontrado`: billed at $0 and stamped "NO ENTREGADO" in Woo. It now joins through `id_asignacion` and, if that read fails, **skips the guard entirely** — marking everything missing is far worse than marking nothing.
...
**The QR/manifest is built from `manifest_items`, never from `products_map`.** `products_map` is a lookup index: it writes the same entry under `product_id` *and* `variation_id` (walking it duplicated every variable product) and collapses the same product ordered by two customers into one entry (last one wins). `utils/manifestItems.js` emits one row per real order line, keyed `${order_id}-${line_item_id}`, and reports `manifest_warnings`: `colisiones` (two lines in one order that would emit the same code — the POS reads them as one product) and `sin_codigo`. **Only real barcodes go in the QR.** The register resolves codes against `siesa_codigos_barras`; it does not understand a bare item (the `f120_id`) or a fabricated code. `buildManifestCode()` *chooses* among the codes the product actually has — it never builds one — and returns `null` when there is none, so the manifest reports it for manual entry. Fabricating the code as f120_id + UM without checking that the row existed broke production: the register rejected every line except fruver, which travels the variable-weight GS1 path instead.
