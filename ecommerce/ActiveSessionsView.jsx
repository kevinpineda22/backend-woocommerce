...
const ActiveSessionsView = ({ onViewDetail }) => {
  ...
  const fetchSessions = async () => {
      try {
          // Nota: No activamos loading aquí para evitar parpadeos en cada actualización real
          const res = await axios.get("https://backend-woocommerce.vercel.app/api/orders/dashboard-activo");
          setSessions(res.data);
          setLoading(false); // Solo quitamos loading la primera vez
      } catch (e) {
  ...
  useEffect(() => {
      ...
      // 2. Suscripción a TODO lo que afecte el dashboard
      const channel = supabase.channel('admin-dashboard-live-updates')
          // A. Cambios en la Sesión (Estado, Inicio, Fin, Items borrados por admin)
          .on(
              'postgres_changes', 
              { event: '*', schema: 'public', table: 'wc_picking_sessions' }, 
              () => {
                  console.log("⚡ Dashboard: Sesión modificada");
                  fetchSessions();
              }
          )
          // B. Cambios en Logs (PICKING REALTIME)
          // INSERT: Picker escaneó algo (Sube progreso)
          // DELETE: Picker deshizo una acción (Baja progreso)
          .on(
              'postgres_changes', 
              { event: '*', schema: 'public', table: 'wc_log_picking' }, 
              (payload) => {
                  console.log(`⚡ Dashboard: Actividad detectada (${payload.eventType})`);
                  fetchSessions();
              }
          )
          // C. Nuevas Asignaciones
          .on(
              'postgres_changes',
              { event: 'INSERT', schema: 'public', table: 'wc_asignaciones_pedidos' },
              () => {
  ...
  if (loading && sessions.length === 0) return <div className="pedidos-empty-list-container"><p>Cargando rutas en vivo...</p></div>;
