...
import {
  ...
  FaUndo // Importamos el icono de deshacer
} from "react-icons/fa";
...
// --- COMPONENTE: TARJETA DE PRODUCTO MODIFICADA ---
const ProductCard = ({ item, orderMap, onAction, isCompleted }) => {
  ...
  // Detectar si fue sustituido y tiene datos
  ...
  return (
    <motion.div 
      layout 
      className={`ec-product-card ${isCompleted ? 'completed' : ''} ${isPartial ? 'partial-scan' : ''} ${isSubstituted ? 'sustituido' : ''}`}
    >
      ...
      <div className="ec-info-col">
        ...
        {/* --- CAMBIO: LISTA DE REQUERIMIENTOS SIEMPRE VISIBLE --- */}
...
const VistaPicker = () => {
  ...
  const confirmSubstitution = (newItem) => {
      ...
      // Actualización optimista completa (inyectando datos del sustituto)
      setSessionData(prev => ({
          ...prev,
          items: prev.items.map(i => {
