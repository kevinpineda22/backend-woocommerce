...
const ProductCard = ({ item, orderMap, onAction, isCompleted }) => {
  // 🛡️ PROTECCIÓN VISUAL: Clamp para evitar números locos
  const scannedRaw = item.qty_scanned || 0;
  const total = item.quantity_total;

  // Si es mixto, el scanned real no puede superar el total (visualmente)
...
const VistaPicker = () => {
  ...
  // ✅ ESTRATEGIA OPTIMIZADA: SUPABASE REALTIME (PUSH)
  // En lugar de consultar cada 5 seg, escuchamos cambios en la DB.
  useEffect(() => {
    if (!pickerInfo?.id) return;

    console.log("🟢 Suscribiendo a cambios en tiempo real...");

    // Canal único para escuchar eventos relevantes al picker
    const channel = supabase
      .channel("picker-updates")
      .on(
        "postgres_changes",
        {
          ...
          schema: "public",
          table: "wc_pickers",
          filter: `id=eq.${pickerInfo.id}`, // Solo cambios en MI usuario
        },
        (payload) => {
          console.log(
            "🔔 Cambio en Picker detectado (Asignación/Estado):",
            payload,
          );
      ...
      .on(
        "postgres_changes",
        {
          event: "*", // INSERT para nuevas asignaciones, UPDATE para cambios
          schema: "public",
          table: "wc_asignaciones_pedidos",
          filter: `id_picker=eq.${pickerInfo.id}`,
        },
        (payload) => {
          console.log("📋 Cambio en Asignación detectado:", payload);
          refreshSessionData(pickerInfo.id);
        },
      )
      .on(
        "postgres_changes",
        {
          ...
          // Filtro dinámico: Si tengo sesión, escucho esa. Si no, escucho cualquiera nueva donde yo sea el picker.
          filter: sessionData?.session_id
            ? `id=eq.${sessionData.session_id}`
            : `id_picker=eq.${pickerInfo.id}`,
        },
        (payload) => {
          console.log("📦 Cambio en Sesión detectado:", payload);
          refreshSessionData(pickerInfo.id);
        },
      )
      // Escuchar cambios administrativos en items (ej. Admin borra un item)
      // Esta tabla suele tener mucho movimiento, filtramos por la sesión actual si existe
      .on(
        "postgres_changes",
        {
          event: "*",
          ...
          filter:
            ...
            sessionData?.active_assignments_ids?.length > 0
              ? `id_asignacion=in.(${sessionData.active_assignments_ids.join(",")})`
              : `id_asignacion=eq.0`,
        },
        (payload) => {
          console.log("📝 Cambio en Logs detectado (Admin action):", payload);
          refreshSessionData(pickerInfo.id);
        },
      )
      .subscribe();
  ...
  }, [pickerInfo?.id, sessionData?.session_id, refreshSessionData]);
    ...
    const setupListener = (sid) => {
      ...
      pollingInterval = setInterval(async () => {
        try {
          ...
          if (
            data &&
            (data.estado === "auditado" || data.estado === "finalizado")
          ) {
            resetSesionLocal();
            window.location.reload();
          }
  ...
  const orderIndexMap = useMemo(() => {
    ...
    const map = {};
    sessionData.orders_info.forEach((ord, idx) => {
      map[ord.id] = idx;
    });
    return map;
  }, [sessionData]);
  ...
  const confirmPicking = async (peso = null) => {
    ...
    // Guardamos la referencia para evitar race conditions
    const itemRef = currentItem;

    // 🛡️ PROTECCIÓN CONTRA DOBLE CLICK / EXCESO
    if ((itemRef.qty_scanned || 0) >= itemRef.quantity_total) {
      console.warn("Intento de picking excediendo cantidad total.");
      return;
    }
    ...
    try {
      ...
      if (isFinished) {
        ...
        closeAllModals();
        setCurrentItem(null); // Limpiamos inmediatamente
      } else {
        if (navigator.vibrate) navigator.vibrate(100);
        // Actualizamos currentItem para siguiente scan en ráfaga
        setCurrentItem((prev) =>
  ...
  const handleManualValidation = async (inputCode) => {
    if (!isOnline) {
      if (
        window.confirm(
          "⚠️ Estás Offline. Validación local deshabilitada. ¿Forzar recolección?",
        )
      ) {
