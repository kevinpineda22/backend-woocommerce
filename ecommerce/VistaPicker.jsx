...
const VistaPicker = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`active-session-updates-${sid}`)
  ...
  // ✅ LISTENER 3: MONITOR DE SALIDA (OPTIMIZADO: SIN POLLING)
  // Estrategia: Realtime + Verificación al enfocar ventana (Ahorra recursos)
  useEffect(() => {
    if (!completedSessionId) return;

    console.log("🔒 Esperando liberación de sesión:", completedSessionId);
    ...
    const checkStatus = async () => {
      ...
      try {
        ...
        if (
          ...
          // Estado final detectado: Liberar
          if (navigator.vibrate) navigator.vibrate([200, 100, 200]);
          localStorage.removeItem("waiting_for_audit_id");
          setCompletedSessionId(null);
          setShowSuccessQR(false);
    ...
    // 1. Canal Realtime (El método principal)
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log("⚡ Cambio de estado detectado por socket");
          checkStatus(); // Verificar inmediatamente al recibir señal
        },
      )
      .subscribe((status) => {
        // Si el socket se reconecta (ej: volvió el internet), verificamos por si nos perdimos algo
        if (status === "SUBSCRIBED") checkStatus();
      });

    // 2. Evento "Wake Up" (Cuando el usuario vuelve a mirar la app)
    // Esto reemplaza al polling. Solo chequea si el usuario desbloquea el cel o cambia de pestaña.
    const onFocus = () => {
        ...
        while (queue.length > 0) {
          ...
          try {
            ...
            if (
              ...
              err.response.data.error.includes("Sesión inválida")
            ) {
