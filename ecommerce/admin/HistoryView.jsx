            ...
            <tr key={sess.id}>
              ...
              <td>
                <div className="hv-cell-acciones">
                  {isPaymentView && (
                    <button
                      className="hv-btn-pay-open"
                      title="Registrar pagos por pedido"
                      onClick={() => onRegisterPayments(sess)}
                    >
                      <FaMoneyBillWave />
                      <span>
                        Registrar pagos
                        {sess.pagos_pedidos ? (
                          <span className="hv-pay-progress">
                            {" "}
                            ({sess.pagos_pedidos.filter((p) => p.metodo_pago).length}/
                            {sess.pedidos.length})
                          </span>
                        ) : (
                          sess.pedidos?.length > 1 ? ` (${sess.pedidos.length})` : ""
                        )}
                      </span>
                    </button>
                  )}
