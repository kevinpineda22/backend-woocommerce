    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        processedRoutes.forEach((route) => {
          ...
          const siblings = processedRoutes.filter(
            (r) =>
              r.id_picker === route.id_picker &&
              r.id !== route.id &&
              !processedIds.has(r.id) &&
              Math.abs(new Date(r.fecha_fin).getTime() - routeTime) <
                5 * 60 * 1000, // 5 min diferencia max
          );
  ...
  if (loading)
    return (
      <div className="card-analitica">
        <p>Cargando historial de rutas...</p>
      </div>
    );
  ...
  return (
    <div className="card-analitica">
      ...
      <p style={{ fontSize: "0.9rem", color: "#666", marginBottom: 20 }}>
        Selecciona un pedido completado para ver la visualización de la ruta
        realizada.
      </p>
      ...
      >
        {routesHistory.map((route) => (
          <div
            ...
            style={{
              border: route.is_multipicker
                ? "2px solid #3b82f6"
                : "1px solid #e2e8f0", // Borde azul para multipicker
              borderRadius: 8,
              padding: 15,
              cursor: "pointer",
          ...
          >
            ...
            >
              {route.is_multipicker ? (
                ...
                <span style={{ fontWeight: "bold", color: "#2d3748" }}>
                  Pedido #{route.id_pedido}
                </span>
              )}
              <span
                style={{ fontSize: "0.8rem", color: "#718096", marginTop: 2 }}
              >
                {new Date(route.fecha_fin).toLocaleDateString()}{" "}
                {new Date(route.fecha_fin).toLocaleTimeString([], {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
              </span>
            ...
            >
              <span>
                ⏱️ {Math.floor(route.tiempo_total_segundos / 60)}m{" "}
                {route.tiempo_total_segundos % 60}s
              </span>
      ...
      <div className="analitica-header">
        ...
        >
          <div>
            ...
            <p>Métricas de rendimiento y auditoría de picking en tiempo real</p>
          </div>
      ...
      {loading ? (
        ...
        <div className="dashboard-grid">
                ...
                <div>
                  ...
                  <div
                    style={{
                      ...
                      fontWeight: "bold",
                      color:
                        globalStats.tasa_exito_global > 95
                          ? "#27ae60"
                          : "#e67e22",
                    }}
                  >
          ...
          >
            <div className="card-title">
              <span>
                📊 Ritmo de Trabajo (Pedidos/Hora) - Detalle de Pickers
              </span>
            </div>
                ...
                >
                  {[100, 75, 50, 25, 0].map((pct) => (
                    ...
                    >
                      ...
                      >
                        {Math.ceil(
                          Math.max(...hourlyData.map((h) => h.pedidos), 5) *
                            (pct / 100),
                        )}
                      </span>
                    </div>
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        background:
                          p.segundos_por_item < 60
                            ? "#2ecc71"
                            : p.segundos_por_item < 100
                              ? "#f1c40f"
                              : "#e74c3c",
                        height: "100%",
                        borderRadius: 4,
                  ...
                  <tr key={p.id}>
                    ...
                    <td>
                      ...
                      <div style={{ fontSize: "0.75rem", color: "#7f8c8d" }}>
                        ...
                        {p.motivo_comun_fallo !== "N/A"
                          ? `Falla freq: ${p.motivo_comun_fallo}`
                          : "Sin fallos"}
                      </div>
                    </td>
                    <td>
                      <div style={{ fontWeight: "bold", color: "#2d3748" }}>
                        {p.segundos_por_item}s
                      </div>
          ...
          <div className="card-analitica" style={{ gridColumn: "1 / -1" }}>
            <div className="card-title">
              <span>
                ⚠️ Productos Problemáticos (No Encontrados / Agotados)
              </span>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 15 }}>
                  ...
                  <div className="log-meta">
                    ...
                    <span style={{ fontSize: "0.75rem" }}>
                      ...
                      {log.wc_asignaciones_pedidos?.nombre_picker || "Picker"}
                      {log.motivo && (
        ...
        >
          ...
          >
            <div className="ap-modal-header">
              <h2>
                <FaMapMarkedAlt /> Análisis de Ruta Detallado
              </h2>
              <button
            ...
            <div className="ap-modal-body">
              ...
              >
                <div className="ap-metric-card">
                  ...
                  >
                    {routeData.metrics.total_pasillos_visitados}
                  </div>
                </div>
                <div className="ap-metric-card">
                  <div
                  ...
                  >
                    {Math.floor(routeData.metrics.total_time / 60)}
                    <span style={{ fontSize: "1.5rem" }}>m</span>{" "}
                    {String(routeData.metrics.total_time % 60).padStart(2, "0")}
                ...
                >
                  ...
                  <div style={{ display: "grid", gap: 10 }}>
                    {routeData.regressions
                      .filter(
                        (r) => r.pasillo !== "S/N" && r.pasillo !== "MESA",
                      ) // [MOD] Ignorar regresiones a zona virtual
                      .map((r, i) => (
                        <div
                          key={i}
                          style={{
              ...
              >
                ...
                >
                  <h3 style={{ margin: 0, fontSize: "1rem", color: "#475569" }}>
                    🗺️ Reproducción de Recorrido en Planta
                  </h3>
                </div>
                  ...
                  >
                    {routeData.route.map((seg, i) => {
                      const maxDuration = Math.max(
                        ...routeData.route.map((s) => s.duration_seconds),
                        1,
                      );
                      const widthPercent =
                        (seg.duration_seconds / maxDuration) * 100;
                      const isRegression =
                        i > 0 &&
                        routeData.route
                          .slice(0, i)
                          .some((prev) => prev.pasillo === seg.pasillo);
                      ...
                      return (
                        ...
                        >
                          ...
                          {(() => {
                            const isCancelled = seg.products.some((p) =>
                              ["retirado", "no_encontrado"].includes(p.accion),
                            );
                            const label =
                              isCancelled && seg.pasillo === "S/N"
                                ? "FAIL"
                                : seg.pasillo === "S/N"
                                  ? "MESA"
                                  : seg.pasillo;
                            const colorBg = isCancelled
                              ? "#fef2f2"
                              : isRegression
                                ? "#fee2e2"
                                : "#eff6ff";
                            const colorText = isCancelled
                              ? "#ef4444"
                              : isRegression
                                ? "#dc2626"
                                : "#2563eb";
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            >
                              ...
                              >
                                {Math.floor(seg.duration_seconds / 60)}m{" "}
                                {seg.duration_seconds % 60}s
                              </span>
                            </div>
                            <div
                              style={{
                            ...
                            >
                              ...
                              {seg.products &&
                                seg.products.some((p) =>
                                  ["retirado", "no_encontrado"].includes(
                                    p.accion,
                                  ),
                                ) && (
                                  <span
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              <div>
                <h4 style={{ color: "#3498db", marginBottom: 5 }}>
                  ⏱️ Eficiencia Global (SPI)
                </h4>
                <p
                  style={{ fontSize: "0.9rem", color: "#555", lineHeight: 1.5 }}
                >
                  <strong>Seconds Per Item (Segundos por Item).</strong> Mide la
                  velocidad promedio.
                  <br />
                  <em>
                    Fórmula: Tiempo Total Trabajado / Total Items Recolectados.
                  </em>
                  <br />
                  <span style={{ fontSize: "0.8rem", color: "#7f8c8d" }}>
                    Menor es mejor (indica mayor rapidez).
                  </span>
                </p>
              </div>

              <div>
                <h4 style={{ color: "#e67e22", marginBottom: 5 }}>
                  👣 Distancia Estimada
                </h4>
                <p
                  style={{ fontSize: "0.9rem", color: "#555", lineHeight: 1.5 }}
                >
                  Cálculo aproximado del recorrido físico del picker basado en
                  los pasillos visitados.
                  <br />
                  <span style={{ fontSize: "0.8rem", color: "#7f8c8d" }}>
                    Útil para saber si un empleado camina innecesariamente más
                    que otros.
                  </span>
                </p>
              </div>
              ...
              <div>
                <h4 style={{ color: "#27ae60", marginBottom: 5 }}>
                  🎯 Tasa de Éxito & Pedidos Perfectos
                </h4>
                <p
                  style={{ fontSize: "0.9rem", color: "#555", lineHeight: 1.5 }}
                >
                  <strong>Tasa de Éxito:</strong> Porcentaje de items
                  encontrados vs solicitados.
                  <br />
                  <strong>Ped. Perfectos:</strong> Porcentaje de pedidos
                  completados sin NINGÚN reporte de agotado o no encontrado.
                </p>
              </div>

              <div>
                <h4 style={{ color: "#8e44ad", marginBottom: 5 }}>
                  📊 Ritmo de Trabajo (Carga)
                </h4>
                <p
                  style={{ fontSize: "0.9rem", color: "#555", lineHeight: 1.5 }}
                >
                  Muestra la cantidad de pedidos que <strong>iniciaron</strong>{" "}
                  en cada hora del día.
                  <br />
                  <span style={{ fontSize: "0.8rem", color: "#7f8c8d" }}>
                    Ayuda a identificar las "Horas Pico" reales de demanda en
                    bodega.
                  </span>
                </p>
              </div>
