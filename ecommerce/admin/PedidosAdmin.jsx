...
const PedidosAdmin = () => {
  ...
  // Cada pedido de la sesión puede pagarse con un método distinto. El modal
  // resuelve toda la UI; acá solo abrimos, cerramos y enviamos el array al
  // endpoint. El endpoint decide si la sesión queda finalizada o parcial.
  const handleOpenPaymentModal = (session) => {
    setPaymentModal({ isOpen: true, session, isProcessing: false });
  };

  const handleClosePaymentModal = () => {
    setPaymentModal({ isOpen: false, session: null, isProcessing: false });
  };
  ...
  const handleConfirmPayments = async (payments) => {
    ...
    try {
      const { data } = await ecommerceApi.post(
        ...
        {
          session_id: paymentModal.session.id,
          payments,
          admin_name: adminName,
          admin_email: adminEmail,
        },
      ...
      showToast(
        data?.session_finalized
          ? `✅ ${payments.length} pago${payments.length > 1 ? "s" : ""} registrado${payments.length > 1 ? "s" : ""}. Sesión finalizada.`
          : `✅ Pago${payments.length > 1 ? "s" : ""} registrado${payments.length > 1 ? "s" : ""} (parcial).`,
        "success",
      );
      fetchPaymentPending();
      fetchHistory();
      ...
      <main className="pedidos-layout-content">
        {currentView === "analitica" ? (
          ...
          <PendingPaymentView
            ...
            onViewDetail={handleViewHistoryDetail}
            onViewManifest={handleViewManifestDirect}
            onRegisterPayments={handleOpenPaymentModal}
          />
        ) : currentView === "history" ? (
          /* VISTA DE HISTORIAL */
          <>
