...
const ManifestSheet = ({
  ...
  const qrValue = items
    .flatMap((item) => {
      ...
      // SKU ≤4 dígitos: 29 + SKU(4d padded) + 0 + peso(5d) + check = 13
      // SKU 5 dígitos:  29 + SKU(5d)        +   peso(5d) + check = 13 (sin separador)
      const cleanCodeGS1 = code.toString().replace(/\+$/, "");

      // Extraer SOLO dígitos del SKU (ignorar letras como "LB", "KG", etc.)
      const numericSku = (item.sku || "").match(/^(\d+)/)?.[1];

      // Si ya es GS1 válido con "29" (solo dígitos, 13-14 chars), usarlo tal cual
      ...
      if (isWeighableProduct(unidad_medida) || isAlreadyGS1_29) {
        ...
        if (numericSku) {
          // Peso en gramos: usar peso_total real si existe, sino derivar de qty
          ...
          // Validar que tenemos un peso numérico real
          if (isNaN(pesoKg) || pesoKg <= 0) {
            // Sin peso válido, no podemos generar GS1 — omitir
            omittedItems.push(item);
            return [];
          }
          ...
          // Normalizar SKU: padear a mínimo 4 dígitos
          const skuPadded = numericSku.length < 4
            ? numericSku.padStart(4, "0")
            : numericSku;

          // Construir código GS1 de peso variable — SIEMPRE EAN-13 (12 + check = 13)
          // SKU 4 dígitos: "29" + SKU(4) + "0" + peso(5) = 12 → + check = 13
          // SKU 5 dígitos: "29" + SKU(5) + peso(5) = 12 → + check = 13 (SIN separador "0")
          // SKU 3 dígitos (padeado a 4): igual que SKU 4
        ...
        // Fallback: solo devolver si es un código numérico válido
        if (/^\d{8,14}$/.test(cleanCodeGS1)) {
          return [cleanCodeGS1];
        }
        // Si no hay código válido, omitir producto
        omittedItems.push(item);
        return [];
      }
          ...
          <table className="manifest-table">
            ...
            <tbody>
              {items.map((item, idx) => {
                ...
                const getValidCode = () => {
                  // ✅ Prioridad 1: Si el barcode ya es un GS1 "29" válido (del picker/SIESA), usarlo tal cual
