  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // 2. Obtener info del usuario actual — REFRESCAR desde Supabase
        const empleadoStr = localStorage.getItem("empleado_info");
        let empleado = empleadoStr ? JSON.parse(empleadoStr) : {};

        // Refrescar perfil desde DB para capturar cambios de rol/sede hechos por admin
        const correo = localStorage.getItem("correo_empleado");
        if (correo) {
          const { data: freshProfile } = await supabase
            .from("profiles")
            .select("role, sede_id, ecommerce_rol")
            .eq("correo", correo)
            .maybeSingle();

          if (freshProfile) {
            empleado = { ...empleado, ...freshProfile };
            localStorage.setItem("empleado_info", JSON.stringify(empleado));
          }
        }
