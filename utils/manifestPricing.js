 ...
 * Versión BACKEND (CJS).
 *
 * ⚠️ MANTENER EN SYNC con `ecommerce/shared/manifestPricing.js` (frontend ESM).
 *
...
/**
 * Devuelve el cobro de una línea, redondeado a entero.
 * @param {object} item — debe traer al menos: price | line_total, qty, unidad_medida, peso_total, sku
 * @returns {number} cobro en pesos (entero)
 */
function calcLineCharge(item) {
  if (!item) return 0;
  ...
  // No pesable o pesable sin peso registrado: line_total como unitario × qty
  const unitFinal =
    parseFloat(item.line_total) ||
    parseFloat(item.price) ||
    parseFloat(item.catalog_price) ||
