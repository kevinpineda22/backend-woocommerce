...
const agruparItemsParaPicking = (orders) => {
  ...
  orders.forEach((order) => {
    ...
    const isPickup = order.shipping_lines?.some(
    ...
    order.line_items.forEach((item) => {
      ...
      const noteMeta = item.meta_data?.find(
        (m) => m.key === "_wcfx_item_note" || m.key === "Nota de preparación",
      );
      ...
      if (!item.is_removed) {
        ...
        // Si hay nota específica del producto (meta_data: _wcfx_item_note), la guardamos con nombre del cliente
        if (itemNote) {
          const nombreCliente = order.billing
