...
exports.registerAction = async (req, res) => {
  ...
  try {
    ...
    // CASO ESPECIAL: RESET (Deshacer / Devolver a Pendientes)
    if (accion === "reset") {
      // Borramos el registro de "recolectado" o "sustituido" de este producto en esta sesión
      const { error } = await supabase.from("wc_log_picking").delete().match({
        id_producto: id_producto_original,
        // Nota: Idealmente deberíamos filtrar también por id_asignacion o sesión,
        // pero como el id_producto es único por línea en el batch, esto funciona.
        // Para mayor seguridad, podríamos buscar primero la asignación.
      });

      // Opción más segura: borrar por ID de asignación si la tienes,
      // pero filtrar por id_producto dentro de los logs creados HOY o asociados al picker funciona.
      // Vamos a hacerlo robusto buscando logs recientes de este producto:

      // 1. Buscar asignaciones de la sesión
    ...
    // ... (LÓGICA NORMAL DE RECOLECCIÓN SIGUE IGUAL) ...
...
// =========================================================================
// 3. BÚSQUEDA INTELIGENTE (SEMÁNTICA + FILTRO PASILLOS)
// =========================================================================
...
exports.searchProduct = async (req, res) => {
  ...
  try {
    ...
    if (original_id && !query) {
      ...
      // 4. BUSCAR EN WOOCOMMERCE POR NOMBRE
      ...
      products = searchResults.filter((p) => {
        ...
        // C. FILTRO PASILLO (El Juez)
        const infoCandidato = obtenerInfoPasillo(p.categories, p.name);

        // Si el pasillo no coincide, se descarta (a menos que sea 'Otros')
        if (
