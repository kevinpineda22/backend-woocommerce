...
// Pipeline del manifiesto/QR. Fuente ÚNICA: el QR del historial admin y el
// del picker salen de la misma función — antes eran dos caminos distintos y
// uno de ellos fabricaba códigos desde el SKU.
const { buildSessionManifest } = require("../services/manifestService");
const {
  getSedeFromWooOrder,
...
// =========================================================
// Detalle completo de auditoría (panel admin / auditor).
//
// El pipeline vive en services/manifestService.js. Este handler ya no tiene
// lógica propia: es traducción HTTP. El payload es el mismo de siempre.
exports.getSessionLogsDetail = async (req, res) => {
  try {
    const payload = await buildSessionManifest({
...
// Manifiesto / QR de salida — MISMA fuente que el historial admin.
//
// El QR del historial es la referencia correcta, así que el picker consume
// exactamente esa lista en vez de armarse una propia desde `sesion-activa`
// (donde el fallback `[item.barcode || item.sku]` emite el SKU como si fuera
// un código de barras, y la caja lo rechaza).
//
// Devuelve SOLO lo que hace falta para imprimir: nada de `products_map`,
// `audit_barcode_index`, logs ni `final_snapshot`. Ese payload de admin en un
// celular con datos móviles frena la operación, que es justo lo que no
// queremos.
exports.getSessionManifest = async (req, res) => {
  try {
    const payload = await buildSessionManifest({
      sessionId: req.query.session_id,
      sedeId: req.sedeId,
      // Las categorías reales solo alimentan la detección fruver del auditor.
      // El manifiesto no las mira: un roundtrip menos a WooCommerce.
    ...
    res.status(200).json({
      ...
      manifest_warnings: payload.manifest_warnings,
    });
  } catch (error) {
