    ...
    const dashboardData = await Promise.all(
      sessions.map(async (sess) => {
        ...
        const { data: logs } = await supabase
          .from("wc_log_picking")
          .select("id_producto, accion, es_sustituto, fecha_registro, nombre_producto, pasillo")
        ...
        // Contamos también los 'no_encontrado' como "gestionados" para la barra de progreso
        const noEncontrados = logs.filter(l => l.accion === "no_encontrado").length;
        
        const completados = recolectados + sustituidos + noEncontrados;
